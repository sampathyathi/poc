...
export default function NavBar(props) {
  ...
  const navItems = items
    .filter((item: { type: string; }) => item.type !== 'search')
    .map((item: { link: any; label: any; }, index: React.Key | null | undefined) => {
      return (
  ...
  return (
    <NavWrapper hasBackground={isMain}>
      <Flex {...a}>
        <a href="/"><img src={logo} alt="" height="40" /></a>
        <Flex {...c}>
        <NavItems>
          {navItems}
        </NavItems>
        <SearchBox className="searchdisplay" pathPrefix={props.pathPrefix} />
        </Flex>
      </Flex>
      <NavControls>
...
const NavWrapper = styled.div<{ hasBackground: boolean }>`
  display: flex;
  background: ${({ hasBackground }) => (hasBackground ? '#227a88' : '')};
`;
